...
contains
   ...
   subroutine tda_singlet_excitations(mol, orbitals, energies, n_occ, n_states, &
      ...
      integer, intent(in), optional :: max_subspace
         ...
         !! solver's own rule assumes; it is decided here rather than
         !! forwarded because a deck's unset `max_subspace` is a zero and the
         !! solver would read that as a subspace of no vectors. Set below the
         !! starting space, it truncates the guess.
      integer, intent(in), optional :: batch
      ...
      ! The starting space, plus the room the old default left for expansion:
      ! `n_solve` vectors an iteration and eight over. Sizing the cap from
      ! `n_guess` rather than from `n_solve` is what keeps the wider start
      ! from being collapsed away after one iteration.
      ...
      ! not more roots; `n_solve` eigenpairs still come back.
      !
