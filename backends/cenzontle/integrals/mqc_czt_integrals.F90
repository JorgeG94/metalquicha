...
module mqc_czt_integrals
   ...
   !! `mqc_czt_direct` and in `build_df_tensor` below.
   ...
   integer, parameter, public :: ERI_PATH_ROTAXIS = 2
      !! Quartets of s, p and L shells through the rotated-axis
      !! McMurchie-Davidson path, `libcint_2e_rotaxis_*`; anything with a d
      !! shell or above stays on Rys, which is the only path that has it.
...
contains
...
#ifdef MQC_WITH_LIBFINT
      ! The rotated-axis path covers s, p and L shells and error-stops on
      ! anything else rather than falling back, so the choice is made here,
      ! per quartet, from the angular momenta alone. It ignores `opt`.
      if (eri_path == ERI_PATH_ROTAXIS) then
         if (quartet_is_sp(shls, bas)) then
            if (cartesian) then
   ...
   pure function quartet_is_sp(shls, bas) result(sp)
      !! Whether every shell of a quartet is s, p or L
      !!
      ...
      sp = all(bas(LIBCINT_ANG_OF, shls + 1) <= 1)
   end function quartet_is_sp
   ...
   subroutine set_eri_path(name, error)
      ...
      !! 'rotaxis' takes the rotated-axis path on every s/p/L quartet and Rys
      !! on the rest; 'auto' is 'rotaxis' where the build has it and 'rys'
   ...
   end subroutine build_czt_molecule
   ...
   subroutine molecule_build(this, atomic_numbers, coordinates, basis, error, &
      ...
      logical, intent(in), optional :: normalize_contractions
         !! See `build_czt_molecule`. Default true.
   ...
   subroutine put_ecp_channel(this, row, atom0, ang, shell, off)
      ...
      !! orbital shell.
      type(czt_molecule_t), intent(inout) :: this
   ...
   function fused_sp_pair(this, ish) result(is_sp)
      ...
      type(czt_molecule_t), intent(in) :: this
   ...
   subroutine build_df_tensor(orb, aux, b, error, omega)
      ...
      type(czt_molecule_t), intent(in) :: orb   !! Orbital basis
   ...
   subroutine shell_pair_bounds(mol, q)
      ...
      !!
      !! A near-duplicate of `schwarz_bounds` in `mqc_czt_direct` rather than
   ...
   subroutine mixed_basis_overlap(orb, aux, overlap, error)
      ...
      type(czt_molecule_t), intent(in) :: orb   !! Orbital basis
      type(czt_molecule_t), intent(in) :: aux   !! Second basis, same atoms
   ...
   subroutine molecule_atom_subset(this, iatom, atom_mol, error)
      ...
      type(czt_molecule_t), intent(out) :: atom_mol
      type(error_t), intent(inout) :: error
