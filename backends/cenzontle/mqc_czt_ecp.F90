...
module mqc_czt_ecp
   ...
   !!
   !! `nbas` stays the orbital count. `mqc_czt_integrals` lays that out;
   ...
   ! link against. Read by `build_czt_molecule`, which refuses a deck naming
...
contains
   ...
   subroutine ecp_matrix(nao, nbas, natm, cartesian, atm, bas, env, &
      ...
      !!
      !! **Plain arrays rather than `czt_molecule_t`**:
      ...
      !!
      !! The loop mirrors `one_electron` in `mqc_czt_integrals`, but shell
      ...
      do ish = 1, nbas
         ...
         do jsh = 1, nbas
            ...
            ! because build_czt_molecule refuses the deck that would set
