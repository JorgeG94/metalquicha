...
module mqc_czt_ri_mp2_gradient
   ...
   !! two-electron derivative terms -- is `mqc_czt_mp2_gradient`'s.
   !!
   !! Bit-reproducible at one thread only: threaded, the Fock accumulators in
   !! `mqc_czt_direct.f90` merge in arrival order, which moves the result by
   !! ~5e-11.
   ...
   use mqc_czt_integrals, only: czt_molecule_t, atom_ao_blocks, &
                                ...
                                metric_inverse_sqrt
   use mqc_czt_cphf, only: cphf_solve, fitted_potential_general, IN_CORE_LIMIT
   ...
   use mqc_czt_mp2_gradient, only: gamma1_intermediates, two_electron_potential, &
                                   two_electron_mp2_terms
   use mqc_czt_ecp, only: ecp_refuses_derivatives
   implicit none
   private

   public :: czt_ri_mp2_gradient
...
contains
   ...
   subroutine czt_ri_mp2_gradient(mol, aux, coeff, orbital_energies, n_occ, &
      ...
      type(xc_context_t), intent(inout), optional :: xc
         ...
         !!    `czt_scf_gradient` has already returned it.
      ...
      ! `czt_scf_gradient` already returned it. Adding it here gives roughly
   ...
   end subroutine czt_ri_mp2_gradient
...
end module mqc_czt_ri_mp2_gradient
