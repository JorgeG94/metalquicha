...
contains
   ...
   subroutine czt_mp2_gradient(mol, coeff, orbital_energies, n_occ, gradient, &
      ...
      !! `czt_scf_gradient` has already returned that -- so the caller adds
      !! the two.
      ...
      type(czt_molecule_t), intent(in), optional :: aux
         ...
         !! different from `czt_ri_mp2_gradient`: its two-particle density is
      ...
      ! hybrid leaves out because `czt_scf_gradient` already returned it.
   ...
   end subroutine czt_mp2_gradient
...
end module mqc_czt_mp2_gradient
