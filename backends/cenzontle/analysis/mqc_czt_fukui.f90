...
module mqc_czt_fukui
   ...
   !!
   !! The three calculations share one `czt_molecule_t`, so they see the
...
contains
   ...
   subroutine fukui_indices(mol, nelec, multiplicity, neutral_density, neutral_energy, &
      ...
      call run_czt_uhf(mol, nelec + 1, 2, max_iter, energy_tol, density_tol, &
   ...
   subroutine state_pt2(mol, aux, coeff, eps, n_occ, frozen, e_corr, error)
      ...
      e_corr = 0.0_dp
      if (present(aux)) then
         call run_czt_ri_mp2(mol, aux, coeff, eps, n_occ, 0.0_dp, m, error, &
                             n_frozen=frozen)
      else
         call run_czt_mp2(mol, coeff, eps, n_occ, 0.0_dp, m, error, n_frozen=frozen)
      end if
      if (error%has_error()) return
   ...
   subroutine state_pt2_open(mol, aux, scf, frozen, e_corr, error)
      ...
      e_corr = 0.0_dp
      if (present(aux)) then
         call run_czt_uri_mp2(mol, aux, scf%orbitals, scf%orbitals_beta, &
