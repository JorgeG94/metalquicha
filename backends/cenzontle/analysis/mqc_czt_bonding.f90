...
module mqc_czt_bonding
   ...
   !!
   !! The pipeline itself lives in `mqc_aambs`, `mqc_czt_quao` and
   !! `mqc_czt_quao_report`. What is here is the assembly: which spaces to
   ...
   use mqc_czt_integrals, only: czt_molecule_t, mixed_basis_overlap
   ...
   use mqc_czt_casci, only: active_space_integrals, run_czt_casci, casci_result_t
   ...
   use mqc_czt_ieda, only: kinetic_decomposition, print_kinetic_decomposition, &
...
contains
   ...
   subroutine no_sharing_analysis(mol, full_quao, valence_quao, n_core, &
      ...
      if (ci_route == NO_SHARING_CI_RESOLVE) then
         ...
         call run_czt_casci(mol, full_quao%orbitals, n_core, n_active, na, nb, &
         ...
         if (inherited) then
            ...
            call run_czt_casci(mol, mo_basis, n_core, n_active, na, nb, &
                               cas, error, verbose=.false.)
            if (error%has_error()) return
            e_psi = cas%energy
            n_total = size(cas%ci_vector)
