...
module mqc_czt_cc
   ...
   type :: cc_result_t
      ...
      real(dp) :: e_mp2 = 0.0_dp
         ...
         !! `run_czt_mp2` exactly. It is the first iteration's energy, so it
...
contains
   ...
   subroutine build_cc_eris_fitted(chem, map, no, nv, eris)
      ...
      !! because `run_czt_ccsd` refuses an unrestricted reference on the
   ...
   subroutine run_czt_ccsd(mol, coeff, orbital_energies, n_occ, frozen, &
   ...
   subroutine mp2_amplitudes(eris, eps, no, nv, t2, e_mp2)
      ...
      !!
      !! Must reproduce `run_czt_mp2` exactly. If it does not, the
