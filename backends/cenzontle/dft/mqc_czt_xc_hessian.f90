...
module mqc_czt_xc_hessian
   ...
   use mqc_czt_ao, only: eval_ao_block, shell_extents, block_significant_aos, &
                         AO_DERIV3_COMP, eval_rho
   use mqc_czt_xc, only: xc_context_t, xc_grid_kernel_quantities, &
...
contains
   ...
   subroutine xc_potential_hessian(ctx, mol, density, pmat, hess, error)
      ...
      !! argued itself into a merge.** That merge (`czt_mp2_gradient`) was
