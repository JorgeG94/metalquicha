...
def pyscf_grid(label, level, prune=None):
...
for label in MOLECULES:
    for level in range(1, 6):
        mol, coords, weights = pyscf_grid(label, level, prune=None)
        exact = mol.natm
...
print(f"{'molecule':>15} {'points':>9} {'max |dx|':>12} {'max rel |dw|':>14}   ")

griddir = sys.argv[1] if len(sys.argv) > 1 else "."
failed = []
cases = ([(label, None, f"grid_{label.strip()}.txt") for label in MOLECULES] +
         [(label, gen_grid.nwchem_prune, f"grid_pruned_{label.strip()}.txt") for label in MOLECULES])
for label, prune, fname in cases:
    path = f"{griddir}/{fname}"
    ours = np.loadtxt(path)
    mol, coords, weights = pyscf_grid(label, 3, prune=prune)

    tag = f"{label}{'/pruned' if prune else ''}"
    if len(ours) != len(weights):
        print(f"{tag:>15} {len(ours):>9}   count mismatch: pyscf has {len(weights)}")
        failed.append(tag)
        continue
    ...
    if not ok:
        failed.append(tag)
    print(f"{tag:>15} {len(ours):>9} {dx:>12.3e} {dw:>14.3e}   {'ok' if ok else 'FAIL'}"
