...
HAND_MAINTAINED = {
    ...
    # Mayer bond orders. Same situation as the Fukui cases: the manifest has
    # nowhere to put a matrix, so what it checks is the ordinary SCF energy and
    # what the case covers is that the property runs end to end -- the deck's
    # own energy is unchanged by asking for it, which is the claim that these
    # cost nothing beyond the SCF. The numbers themselves are pinned in
    # test/test_mqc_mayer_bond_orders.f90 against PySCF and numpy, generated by
    # tools/cpu_validation/mayer_bond_orders_reference.py.
    #
    # The O2 case is the one worth having: it is unrestricted, and the
    # open-shell formula is a sum over spins rather than the closed-shell
    # expression applied to the total density. PySCF over the same density
    # gives 1.854271 by the right formula and 1.406158 by the wrong one, so a
    # run whose table says 1.8543 has taken the right branch.
    "cpu/mqc/bond_orders/cpu_water_6-31g_mayer.json",
    "cpu/mqc/bond_orders/cpu_o2_cc-pvdz_mayer.json",
    ...
    "cpu/mqc/interaction_energy/ie_prism_sto-3g_l3_vmfc.json",
...
PRESERVED_TESTS = [
    ...
    },
    {
        ...
        "type": "fragmented",
        "reference_note": "reference is the sum of the five counterpoise-corrected water-5 dimer corrections of the ordinary VMFC(2) run of the same deck. 21 of 51 rows are computed: each pair builds its own two ghosted rows, so the reduction carries over unchanged. Level 3 is the next entry.",
    },
    {
        "name": "InteractionEnergy water prism STO-3G, VMFC(3), last water (CPU)",
        "input": "inputs/cpu/mqc/interaction_energy/ie_prism_sto-3g_l3_vmfc.json",
        "expected_interaction_energy": -0.003221678201,
        "type": "fragmented",
        "reference_note": "reference is the Valiron-Mayer sum over the fifteen terms holding water 5 -- five dimers and ten trimers -- each term's increment the alternating sum of its subsets' energies all in that term's own basis, evaluated in Python from the 191-row fragment table of the ordinary VMFC(3) run of the same deck rather than by the code's own recursion. 121 of 191 rows are computed. Before the ghosted-row subset keys kept their ghosts, a VMFC(3) trimer subtracted its pairs' monomers in the pair basis instead of its own, and this driver refused counterpoise above level 2.",
    },
    {
