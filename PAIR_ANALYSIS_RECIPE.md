...
Measured on `feat/fmo-peptide` (off `origin/feat/fmo-neutral-cut`), same build and same
single thread. Everything in Part I still holds; this part says what changed and what the
...
The geometry is `validation/inputs/sample_inputs/gly3_water.xyz` — the shipped glycine
tripeptide with one water hydrogen-bonded to the residue-2 carbonyl at 1.893 Å.
...
3. **`efmo` still has no covalent-cut check** and the documentation still claimed it did;
   `efmo.rst` now says so. Do not point EFMO at a protein.
