...
module test_mqc_libcint_projection
   ...
   use mqc_libcint_projection, only: cross_overlap, project_occupied
   use mqc_error, only: error_t
   implicit none
   private

   public :: collect_mqc_libcint_projection_tests

   real(dp), parameter :: ANG = 1.8897261254578281_dp

contains

   subroutine collect_mqc_libcint_projection_tests(testsuite)
   ...
   end subroutine collect_mqc_libcint_projection_tests
   ...
   subroutine test_electron_count(error)
      ...
      type(libcint_molecule_t) :: small, target_mol
...
end module test_mqc_libcint_projection

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_libcint_projection, only: collect_mqc_libcint_projection_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_libcint_projection", collect_mqc_libcint_projection_tests)]
