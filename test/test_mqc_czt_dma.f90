...
module test_mqc_czt_dma
   ...
   use mqc_czt_dma, only: dma_result_t, distributed_multipoles, expansion_points
   use mqc_czt_screening, only: screening_grid
...
!$ use omp_lib, only: omp_get_max_threads, omp_set_num_threads
   ...
   public :: collect_mqc_czt_dma_tests
...
contains
   ...
   subroutine collect_mqc_czt_dma_tests(testsuite)
   ...
   end subroutine collect_mqc_czt_dma_tests
   ...
   subroutine dimer_multipoles(dma, nelec, err, ok)
      ...
      ok = .false.
      nelec = sum(Z_DIMER)
      call build_czt_molecule(Z_DIMER, SYM_DIMER, GEO_DIMER, "sto-3g", mol, err)
      if (err%has_error()) then
         call mol%destroy()
         return
      end if
      call run_czt_rhf(mol, nelec, 200, 1.0e-11_dp, 1.0e-9_dp, .false., scf, err)
...
end module test_mqc_czt_dma

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_czt_dma, only: collect_mqc_czt_dma_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_dma", collect_mqc_czt_dma_tests)]
