module test_mqc_czt_rcc
   ...
   !! whole reason `mqc_czt_cc` was kept when this module was added, and it
...
contains
   ...
   subroutine collect_mqc_czt_rcc_tests(testsuite)
   ...
   subroutine both_paths(basis, frozen, spin, spatial, ok, triples)
      ...
      call run_czt_rccsd(mol, scf%orbitals, scf%orbital_energies, 5, frozen, &
...
program tester
   ...
   use test_mqc_czt_rcc, only: collect_mqc_czt_rcc_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_rcc", collect_mqc_czt_rcc_tests)]
