module test_mqc_czt_cc
   ...
   use mqc_czt_cc, only: cc_result_t, run_czt_ccsd, spin_orbital_integrals, &
                         so_map_t, build_so_map, so_source_t
   implicit none
   private
   public :: collect_mqc_czt_cc_tests

   real(dp), parameter :: ANG = 1.8897261254578281_dp

contains

   subroutine collect_mqc_czt_cc_tests(testsuite)
   ...
   end subroutine collect_mqc_czt_cc_tests
   ...
   subroutine test_mp2_identity(error)
      ...
      call run_czt_mp2(mol, scf%orbitals, scf%orbital_energies, 5, scf%energy, &
                       mp2, err)
      call check(error,.not. err%has_error(), "conventional MP2 must run: "//err%get_full_trace())
      if (allocated(error)) return

      call run_czt_ccsd(mol, scf%orbitals, scf%orbital_energies, 5, 0, &
   ...
   subroutine test_frozen(error)
      ...
      if (allocated(error)) return
      call run_czt_ccsd(mol, scf%orbitals, scf%orbital_energies, 5, 1, &
   ...
   subroutine test_uccsd_closed_shell(error)
      ...
      call run_czt_rhf(mol, 10, 200, 1.0e-11_dp, 1.0e-9_dp, .false., rscf, err, &
                       in_core=.true.)
      call check(error,.not. err%has_error() .and. rscf%converged, "RHF must converge")
      if (allocated(error)) return

      call run_czt_uhf(mol, 10, 1, 200, 1.0e-11_dp, 1.0e-9_dp, .false., uscf, err, &
                       in_core=.true.)
      call check(error,.not. err%has_error() .and. uscf%converged, "UHF must converge")
      if (allocated(error)) return

      call run_czt_ccsd(mol, rscf%orbitals, rscf%orbital_energies, 5, 0, &
                        60, 1.0e-10_dp, .false., .false., rcc, err)
      call check(error,.not. err%has_error() .and. rcc%converged, "CCSD must converge")
      if (allocated(error)) return

      call run_czt_ccsd(mol, uscf%orbitals, uscf%orbital_energies, 5, 0, &
   ...
   subroutine test_uccsd_open_shell(error)
      ...
      call build_czt_molecule(Z, SYM, c, "sto-3g", mol, err)
      call check(error,.not. err%has_error(), "OH must build")
      if (allocated(error)) return

      call run_czt_uhf(mol, 9, 2, 200, 1.0e-11_dp, 1.0e-9_dp, .false., scf, err, &
                       in_core=.true.)
   ...
   subroutine test_uccsd_refuses_fitting(error)
      ...
      c = reshape([0.0_dp, 0.0_dp, 0.0_dp, &
                   0.0_dp, 0.0_dp, 0.9697_dp*ANG], [3, 2])
      call build_czt_molecule(Z, SYM, c, "sto-3g", mol, err)
      call build_czt_molecule(Z, SYM, c, "cc-pvdz-rifit", aux, err)
   ...
   subroutine test_uccsd_frozen_beta(error)
      ...
      call run_czt_ccsd(mol, scf%orbitals, scf%orbital_energies, 5, 5, &
                        60, 1.0e-10_dp, .false., .false., ucc, err, &
                        coeff_b=scf%orbitals_beta, &
                        orbital_energies_b=scf%orbital_energies_beta, n_occ_b=4)
      call mol%destroy()
   ...
   subroutine test_uccsd_empty_beta(error)
      ...
      call run_czt_uhf(mol, 3, 2, 200, 1.0e-11_dp, 1.0e-9_dp, .false., scf, err, &
                       in_core=.true.)
      call check(error,.not. err%has_error() .and. scf%converged, "UHF must converge")
      if (allocated(error)) return

      call run_czt_ccsd(mol, scf%orbitals, scf%orbital_energies, 2, 1, &
                        60, 1.0e-10_dp, .false., .false., ucc, err, &
                        coeff_b=scf%orbitals_beta, &
...
end module test_mqc_czt_cc

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_czt_cc, only: collect_mqc_czt_cc_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_cc", collect_mqc_czt_cc_tests)]
