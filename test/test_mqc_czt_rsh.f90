module test_mqc_czt_rsh
   ...
   use mqc_czt_rhf, only: SCF_GUESS_SAD
   use mqc_czt_atomic_guess, only: build_atomic_guess, clear_atomic_cache
   implicit none
   private
   public :: collect_mqc_czt_rsh_tests

   real(dp), parameter :: ANG = 1.8897261254578281_dp

contains

   subroutine collect_mqc_czt_rsh_tests(testsuite)
   ...
   end subroutine collect_mqc_czt_rsh_tests
...
end module test_mqc_czt_rsh

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_czt_rsh, only: collect_mqc_czt_rsh_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_rsh", collect_mqc_czt_rsh_tests)]
