...
module test_mqc_czt_esp
   ...
   public :: collect_mqc_czt_esp_tests

   real(dp), parameter :: ANG = 1.8897261254578281_dp

contains

   subroutine collect_mqc_czt_esp_tests(testsuite)
   ...
   end subroutine collect_mqc_czt_esp_tests
...
end module test_mqc_czt_esp

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_czt_esp, only: collect_mqc_czt_esp_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_esp", collect_mqc_czt_esp_tests)]
