...
!$ use omp_lib, only: omp_get_max_threads, omp_set_num_threads
   ...
   implicit none
   private
   public :: collect_mqc_czt_mp2_tests
...
contains
   ...
   subroutine collect_mqc_czt_mp2_tests(testsuite)
   ...
   subroutine water_mp2(basis, frozen, mp2, err)
      ...
      call run_czt_rhf(mol, 10, 200, E_TOL, D_TOL, .false., scf, err)
      if (err%has_error()) return
   ...
   subroutine test_ri(error)
      ...
      c = reshape([0.0_dp, 0.0_dp, 0.0_dp, &
                   0.0_dp, 0.757_dp*ANG, 0.587_dp*ANG, &
                   0.0_dp, -0.757_dp*ANG, 0.587_dp*ANG], [3, 3])
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], c, "cc-pvdz", mol, err)
   ...
   subroutine water_gradient(basis, frozen, gradient, err, block_bytes, force_blocked)
      ...
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], WATER_B, basis, mol, err)
      if (err%has_error()) return
      call run_czt_rhf(mol, 10, 300, 1.0e-14_dp, 1.0e-12_dp, .false., scf, err)
      if (err%has_error()) return
      call czt_mp2_gradient(mol, scf%orbitals, scf%orbital_energies, 5, &
   ...
   subroutine frozen_energy_at(coords, energy, err)
      ...
      energy = 0.0_dp
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], coords, "sto-3g", mol, err)
      if (err%has_error()) return
      call run_czt_rhf(mol, 10, 300, 1.0e-13_dp, 1.0e-11_dp, .false., scf, err)
      if (err%has_error()) return
      call run_czt_mp2(mol, scf%orbitals, scf%orbital_energies, 5, scf%energy, &
                       mp2, err, n_frozen=1)
      if (err%has_error()) return
...
!$    call omp_set_num_threads(saved)
   ...
   subroutine water_ri_gradient(basis, aux_basis, frozen, gradient, err, force_direct)
      ...
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], WATER_B, basis, mol, err)
      if (err%has_error()) return
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], WATER_B, aux_basis, &
                              aux, err)
      if (err%has_error()) return
      call run_czt_rhf(mol, 10, 300, 1.0e-14_dp, 1.0e-12_dp, .false., scf, err)
      if (err%has_error()) return
   ...
   subroutine ri_frozen_energy_at(coords, energy, err)
      ...
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], coords, "sto-3g", mol, err)
      if (err%has_error()) return
      call build_czt_molecule([8, 1, 1], ["O ", "H ", "H "], coords, &
...
program tester
   ...
   use test_mqc_czt_mp2, only: collect_mqc_czt_mp2_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_mp2", collect_mqc_czt_mp2_tests)]
