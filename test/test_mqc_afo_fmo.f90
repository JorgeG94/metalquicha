...
contains
   ...
   subroutine test_gamess_gly3_water_er(error)
      ...
      !! -762.3153911672.
      !!
      !! In vacuo because GAMESS's FMO2 field is the exact ESP, which a detached
      !! bond is refused with here. Its FMO2 total with that field is
      !! -762.311281570 with ER and -762.311027578 with Boys, against the
      !! molecule's -762.311670856.
      type(error_type), allocatable, intent(out) :: error
      type(error_t) :: err
      type(fmo_options_t) :: opts
      type(fmo_result_t) :: res
