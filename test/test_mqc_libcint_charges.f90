...
module test_mqc_libcint_charges
   ...
   public :: collect_mqc_libcint_charges_tests

   real(dp), parameter :: ANG = 1.8897261254578281_dp

contains

   subroutine collect_mqc_libcint_charges_tests(testsuite)
   ...
   subroutine water(mol, err)
      ...
      call build_libcint_molecule(z, symbols, c, "sto-3g", mol, err)
...
end module test_mqc_libcint_charges

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_libcint_charges, only: collect_mqc_libcint_charges_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_libcint_charges", collect_mqc_libcint_charges_tests)]
