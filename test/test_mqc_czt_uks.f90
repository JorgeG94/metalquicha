module test_mqc_czt_uks
   ...
   use mqc_czt_xc, only: xc_context_t, xc_context_create, xc_add_potential, &
                         xc_add_potential_uks, xc_available
   implicit none
   private
   public :: collect_mqc_czt_uks_tests
...
contains
   ...
   subroutine collect_mqc_czt_uks_tests(testsuite)
   ...
   end subroutine collect_mqc_czt_uks_tests
   ...
   subroutine compare_closed_shell(functional, error)
      ...
      if (allocated(error)) return
      call run_czt_rhf(mol, 10, 100, E_TOL, D_TOL, .false., rks, err, xc=xc_r)
      ...
      if (allocated(error)) return
      call run_czt_uhf(mol, 10, 1, 30, E_TOL, D_TOL, .true., uks, err, xc=xc_u)
   ...
   subroutine test_electron_count(error)
      ...
      call run_czt_uhf(mol, 9, 2, 60, E_TOL, D_TOL, .false., uks, err, xc=xc)
      call check(error,.not. err%has_error(), "the unrestricted SCF must run: "//err%get_full_trace())
      if (allocated(error)) return
...
end module test_mqc_czt_uks

program tester
   use, intrinsic :: iso_fortran_env, only: error_unit
   use testdrive, only: run_testsuite, new_testsuite, testsuite_type
   use test_mqc_czt_uks, only: collect_mqc_czt_uks_tests
   implicit none
   integer :: stat, is
   ...
   stat = 0
   testsuites = [new_testsuite("mqc_czt_uks", collect_mqc_czt_uks_tests)]
