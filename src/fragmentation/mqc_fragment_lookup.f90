!! Fast fragment index lookup using hash table
...
contains
   ...
   subroutine fragment_lookup_init(this, estimated_entries)
      ...
      this%table_size = next_prime(int(estimated_entries*1.3_8))
   ...
   subroutine fragment_lookup_insert(this, monomers, n, fragment_idx)
      ...
      integer :: bucket, i
      type(hash_entry_t), pointer :: entry, new_entry
   ...
   function fragment_lookup_find(this, monomers, n) result(idx)
      ...
      if (allocated(this%table(bucket)%key)) then
         if (arrays_equal(this%table(bucket)%key, sorted_key, n)) then
            idx = this%table(bucket)%value
            return
         end if
         entry => this%table(bucket)%next
         do while (associated(entry))
            if (arrays_equal(entry%key, sorted_key, n)) then
               idx = entry%value
               return
            end if
            entry => entry%next
   ...
   pure function arrays_equal(a, b, n) result(equal)
