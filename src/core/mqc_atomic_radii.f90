...
module mqc_atomic_radii
   !! The atomic radii the program uses, gathered so that a caller picks a
   !! parametrisation deliberately instead of finding whichever table happened
   !! to be nearest.
   !!
   !! There is no single "atomic radius". These tables disagree with each other
   !! by tenths of an Angstrom because they were fitted to different things, and
   !! swapping one for another silently changes results. So they are kept apart
   !! and named for their source rather than merged into a default:
   !!
   ...
   !!
   !! The Bragg-Slater radii that size the DFT partition cells are deliberately
   !! *not* here: they live in `mqc_dft_radial_data`, which is generated by
   !! `tools/dft_grid/gen_radial_fortran.py` and would be rewritten by the next
   !! regeneration. Reach them through `mqc_dft_radial`'s `bragg_radius`.
   !!
   !! Every accessor returns Angstrom and reports an unknown element rather than
   !! substituting a number, because for all four of these an invented radius
   !! shows up as an invented bond or an invented cavity rather than as an error.
   ...
   real(dp), parameter :: EMSLEY_RADII(MAX_Z_EMSLEY) = [ &
      ...
      !! anything beyond is refused, because silently inventing a radius would
      !! silently invent or drop a bond.
...
contains
   ...
   pure function vdw_radius_geodesic(atomic_number) result(radius)
      ...
      !!
      !! Unlike the others this one does substitute a default, because that is
      !! what GAMESS does and the screening grid is a fitting aid rather than a
      !! result: a slightly wrong sphere moves sample points, not an energy.
      !! Elements inside the table's range but absent from it (the noble gases
      !! and Li..Be) are zero there, and take the default too.
      integer, intent(in) :: atomic_number
      real(dp) :: radius
